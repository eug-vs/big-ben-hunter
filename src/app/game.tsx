...
async function verifyResult(key: openpgp.Key, { signed, result }: { signed: string, result: number }) {
  ...
  if (result !== localResult) throw new Error(`Result does not match: server suggested ${result}, but computed ${localResult} locally`);
}
...
export default function Game({ armoredPublicKey }: Props) {
  ...
  return (
    <div>
      ...
      {key && (
        <button className="p-2 m-4 bg-red-500" onClick={handleClick}>
          Click me
        </button>
      )}
